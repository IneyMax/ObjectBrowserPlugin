...
static FAutoConsoleCommandWithWorldArgsAndOutputDevice CmdPrintClassData(
	...
	FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateStatic(&FObjectBrowserUtils::PrintClassDetails)
);
...
FString FObjectBrowserUtils::GetModulePathForClass(UClass* InClass)
{
	FString ModulePath;
...
FString FObjectBrowserUtils::GetModuleNameForClass(UClass* InClass)
{
	FString ModuleName;
...
TSharedPtr<class IPlugin> FObjectBrowserUtils::GetPluginForClass(UClass* InClass)
{
...
bool FObjectBrowserUtils::IsGameModuleClass(UClass* InClass)
{
	bool bResult = false;
	// Find module name from class
	if( InClass )
...
void FObjectBrowserUtils::CollectSourceFiles(UClass* InClass, TArray<FString>& OutSourceFiles)
{
...
void FObjectBrowserUtils::SetClipboardText(const FString& ClipboardText)
{
	UE_LOG(LogObjectBrowser, Log, TEXT("Clipboard set to:\n%s"), *ClipboardText);
...
void FObjectBrowserUtils::ShowBrowserInfoMessage(FText InText, SNotificationItem::ECompletionState InType)
{
...
bool FObjectBrowserUtils::TryUpdateDefaultConfigFile(UObject* Instance)
{
	if (!Instance)
	{
		return false;
...
void FObjectBrowserUtils::PrintClassDetails(const TArray<FString>& InArgs, UWorld* InWorld, FOutputDevice& InLog)
{
	if (InArgs.Num() != 1)
	{
...
void FObjectBrowserUtils::PrintPropertyDetails(const TArray< FString >& InArgs, UWorld* InWorld, FOutputDevice& InLog)
{
	if (InArgs.Num() != 2)
	{
		InLog.Log(TEXT("Invalid number of arguments"));
